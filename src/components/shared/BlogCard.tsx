...
const BlogCard = (props: PostEdge) => {
	...
	return (
		<Link
			to={`/blog/${node.slug}`}
			key={node.id}
			className="flex aspect-square w-full flex-col justify-between gap-2 rounded border border-dark-400 bg-dark-400 p-2 text-light transition-all duration-300 hover:-translate-y-1">
